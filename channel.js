...
function getImageUrl(url, channel) {
    ...
    if (index != -1) {
        ...
        if (index != -1) {
            ...
            bbsResUrl = 'http://jbbs.shitaraba.net/bbs/rawmode.cgi/' + genre + '/' + board + '/' + thread + '/1';
        } else {
            var elem = url.split('/');
            var genre = elem[3];
            var board = elem[4];
...
function getImageUrlBBSTop($bbs) {
    ...
    .each(function(index, element){
        if (element.attribs.src !== undefined &&
            ...
            element.attribs.src.match('/pukiwiki.png/') == null
        ) {
...
function getImageUrlWiki($bbs) {
    ...
    .each(function(index, element){
        if (element.attribs.src !== undefined &&
            element.attribs.src.indexOf('pukiwiki.png') == -1 &&
            element.attribs.src.indexOf('rss.png') == -1
        ) {
            if (element.attribs.src !== undefined && element.attribs.src.startsWith('http')) {
                return element.attribs.src;
            } else {
...
function getImageUrlBBSLocal($bbs) {
    ...
    //youtube の取得
    // http://i.ytimg.com/vi/iiAiavN7fQs/mqdefault.jpg
    // var html = $bbs.html();
    // var result = html.match(/twitter.com[/]([a-zA-Z0-9_]+)/i);
